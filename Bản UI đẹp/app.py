...
try:
    ...
    from widgets import (PathEntry, FileTreeView, ActionButtons, StatusBar, 
                        InputDialog, InfoDialog, GuideWindow)
...
class FileExplorerApp:
    ...
    def _setup_window(self):
        self.root.title(f"🗂️ {Config.APP_NAME} v{Config.APP_VERSION}")
    ...
    def _apply_professional_styling(self):
        """Apply professional styling to the entire application"""
        style = ttk.Style()
        ...
        # Professional entry styling  
        style.configure('TEntry',
                       borderwidth=1,
                       relief='solid',
                       focuscolor='#007bff')
        
        # Notebook styling for future use
    ...
    def _create_widgets(self):
        try:
            ...
            # Path entry with enhanced styling
            self.path_entry = PathEntry(main_container, self._on_path_change)
            self.path_entry.grid(row=0, column=0, sticky="ew", pady=(0, 8))
            
            # File tree view with enhanced styling
            self.tree_view = FileTreeView(main_container, self._on_item_select, self._on_item_activate)
            self.tree_view.grid(row=1, column=0, sticky="nsew", pady=(0, 8))
            ...
            actions = {
                ...
                'move': self._move_items,
                'info': self._show_item_info,
                'toggle_hidden': self._toggle_hidden_files
            }
            
            # Action buttons with enhanced styling
            self.action_buttons = ActionButtons(main_container, actions)
            self.action_buttons.grid(row=2, column=0, sticky="ew", pady=(0, 8))
    ...
    def _setup_menu(self):
        try:
            ...
            file_menu.add_command(label="✂️ Cắt", command=self._cut_items, accelerator="Ctrl+X")
            ...
            file_menu.add_separator()
            file_menu.add_command(label="🗑️ Xóa", command=self._delete_items, accelerator="Delete")
            file_menu.add_command(label="✏️ Đổi tên", command=self._rename_item, accelerator="F2")
            file_menu.add_separator()
            file_menu.add_command(label="❌ Thoát", command=self._quit_app, accelerator="Ctrl+Q")
            
            # View menu with enhanced styling
            view_menu = tk.Menu(menubar, tearoff=0, font=('Segoe UI', 9))
            menubar.add_cascade(label="👁️ View", menu=view_menu)
            view_menu.add_checkbutton(label="👁️ Hiển thị file ẩn", command=self._toggle_hidden_files)
            view_menu.add_command(label="🔄 Làm mới", command=self._refresh, accelerator="F5")
            view_menu.add_separator()
            view_menu.add_command(label="📖 Hướng dẫn sử dụng", command=self._show_guide_window, accelerator="F1")
            
        except Exception as e:
            Logger.log_error(f"Error creating menu: {e}")
    
    def _show_guide_window(self):
        """Show the professional guide window"""
        try:
            if not self.guide_window:
                self.guide_window = GuideWindow(self.root)
            self.guide_window.show()
        except Exception as e:
            Logger.log_error(f"Error showing guide window: {e}")
            self._show_error("Lỗi", f"Không thể hiển thị hướng dẫn: {str(e)}")
    ...
    def _setup_shortcuts(self):
        try:
            shortcuts = {
                ...
                "<Control-f>": lambda e: self._show_search_dialog(),
                "<Control-Shift-n>": lambda e: self._open_new_window(),
                "<Delete>": lambda e: self._delete_items(),
                "<F2>": lambda e: self._rename_item(),
                "<F5>": lambda e: self._refresh(),
                "<F1>": lambda e: self._show_guide_window(),
    ...
    def _on_path_change(self, path: str):
        try:
            if path == "UP":
                ...
                self._show_guide_window()  # Changed to show guide window
            else:
    ...
    def _show_drives_menu(self):
        try:
            drives_menu = tk.Menu(self.root, tearoff=0, font=('Segoe UI', 9))
    ...
    def _show_text_preview(self, file_path: str):
        try:
            # Create professional preview window
            ...
            # Header
            header_frame = tk.Frame(preview_window, bg="#007bff", height=50)
            ...
            main_frame = tk.Frame(preview_window, bg="#f8f9fa")
            ...
            footer_frame = tk.Frame(preview_window, bg="#f8f9fa")
            ...
            x = (preview_window.winfo_screenwidth() // 2) - (450 // 2)
            y = (preview_window.winfo_screenheight() // 2) - (325 // 2)
            preview_window.geometry(f"900x650+{x}+{y}")
    ...
    def _rename_item(self):
        try:
            if not self.selected_items:
                InputDialog.show_warning("⚠️ Cảnh báo", "Vui lòng chọn một mục để đổi tên.")
                return
